...
An empty array literal `var results` is initially created. An anonymous function is then passed to each async function, which updates `results` via `results.push()` with data unique to each async function. 'results' is accessible and manipulatable inside each async function (despite being called asyncronously) because of the closure formed in `asyncCaller()`'s signature, enabling `results` to serve as the cache of data returned by the async functions.  When the number of values, via `results.length`, stored in `results` equals the number of async functions, via `Array.prototype.splice.call(arguments, 1).length`, initially passed to asyncCaller(), the callback (passed as the first parameter to `asyncCaller()`) is called, now with a full `results` array: `callback(results)`.
